...
function bindDragDrop() {
    ...
    document.addEventListener('drop', async (e) => {
        ...
        if (!isMarkdown && (commonCodeExts.includes(ext) || (typeof Prism !== 'undefined' && Prism.languages[ext]))) {
            ...
            if (typeof showToast === 'function') showToast(t('toast.codeExpandedWithFile').replace('${fileName}', fileName));
            return;
        }
...
async function loadFile(file, handle = null) {
    ...
    try {
        ...
        AppState.documentId = typeof crypto !== 'undefined' && crypto.randomUUID ? crypto.randomUUID() : `doc_${Date.now()}_${Math.floor(Math.random()*10000)}`;
        ...
        // ファイル読み込み時は、改ページログをなるべく早く出力する（画像等のロードを0.5秒だけ待機）
        setTimeout(() => {
            if (typeof window.renderPageBreaks === 'function') {
                console.log("[File Load] ファイル読み込み時の改ページ位置を計算します...");
                window.renderPageBreaks();
            }
        }, 500);
    ...
    try {
        ...
        if (window.__TAURI__) {
            ...
            if (selected) {
                ...
                if (typeof updateTitle === 'function') updateTitle();
                if (typeof showToast === 'function') showToast(t('toast.fileOpenedWithFile').replace('${fileName}', fileName));
            }
            return;
        }
        ...
        if (e.name !== 'AbortError') {
            ...
            if (typeof showToast === 'function') showToast(t('toast.fileOpenFailed'), "error");
        }
    }
}
...
async function saveFile() {
    ...
    // アノテーションデータを末尾に付加する
    let text = typeof getEditorText === 'function' ? getEditorText() : "";
    text = _attachAnnotationData(text);
    ...
    try {
        ...
        if (window.__TAURI__) {
            ...
            if (targetPath) {
                ...
                AppState.isModified = false;
                if (typeof showToast === 'function') showToast(t('toast.saved'));
            }
            return;
        }
        ...
        if (e.name !== 'AbortError') {
            console.error(e);
            if (typeof showToast === 'function') showToast(t('toast.saveFailed'), "error");
        }
    }
}
    ...
    try {
        ...
        if (window.__TAURI__) {
            ...
            if (targetPath) {
                ...
                if (typeof updateTitle === 'function') updateTitle();
                if (typeof showToast === 'function') showToast(t('toast.savedAs'));
            }
            return;
        }
 ...
 * @param {number} [targetIndex=0] - 抽出するSVGブロックのインデックス（0始まり）
...
function extractSvgContent(text, targetIndex = 0) {
    ...
    const safeIndex = (targetIndex >= 0 && targetIndex < matches.length) ? targetIndex : 0;
    const match = matches[safeIndex];
    const svgText = match[1];

    // 対象ブロック以外にテキストがあるかどうかチェック
    const outside = text.replace(match[0], '').trim();
    return {
        svgText,
...
async function saveSvgMode(mode) {
    ...
    // 編集中のSVGブロックのインデックスを取得（複数ブロック持つドキュメント対応）
    const svgIndex = (window.currentEditingSVG && typeof window.currentEditingSVG.svgIndex === 'number')
        ? window.currentEditingSVG.svgIndex
        : 0;
    const result = extractSvgContent(text, svgIndex);

    // SVGコードブロックが見つからない場合
    if (!result) {
        if (typeof showToast === 'function') showToast(t('error.svgBlockNotFound'), 'error');
        return;
    }
    ...
    try {
        ...
        if (window.__TAURI__) {
            ...
            if (typeof updateTitle === 'function') updateTitle();
            const msg = mode === 'save-as' ? t('toast.savedAs') : t('toast.saved');
        ...
        if (e.name !== 'AbortError') {
            ...
            if (typeof showToast === 'function') showToast(t('error.svgSaveFailed'), 'error');
        }
    }
}
    ...
    try {
        if (isTauri) {
            ...
            if (!webviewWindow || !webviewWindow.WebviewWindow) {
                ...
                alert(t('alert.newWindowApiMissing'));
                return;
            }
...
/**
 * アノテーションデータをMarkdownテキストに付加する
 * 既存のアノテーションコメントを除去したうえで新しいデータを末尾に追記する
 * @param {string} text - エディタのテキスト
 * @returns {string} アノテーションデータが付加されたテキスト
 */
function _attachAnnotationData(text) {
    // 既存のアノテーションコメントを除去
    const ANNO_PATTERN = /\n?<!-- ANNOTATION_DATA:([\s\S]*?)-->/g;
    let cleanText = text.replace(ANNO_PATTERN, '');

    // AnnotationLayerからSVGデータを取得
    if (typeof window.AnnotationLayer !== 'undefined') {
        const svgData = window.AnnotationLayer.getSVGData();
        if (svgData) {
