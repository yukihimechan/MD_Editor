...
class SVGTextAlignToolbar extends SVGToolbarBase {
    ...
    findTextGroup(el) {
        if (!el) return null;
        if (el.attr('data-tool-id') === 'shape-text-group' || el.attr('data-tool-id') === 'bubble') return el;
        const parent = el.parent();
        if (parent && parent.attr && (parent.attr('data-tool-id') === 'shape-text-group' || parent.attr('data-tool-id') === 'bubble')) return parent;
    ...
    updateTextPosition(target, skipSync = false) {
        ...
        if (target.attr('data-tool-id') === 'shape-text-group' || target.attr('data-tool-id') === 'bubble') {
        ...
        let sBox;
        if (shape) {
            if (shape.attr('data-tool-id') === 'bubble') {
                const bw = parseFloat(shape.attr('data-width'));
                const bh = parseFloat(shape.attr('data-height'));
                const bx = parseFloat(shape.attr('data-rect-x'));
                const by = parseFloat(shape.attr('data-rect-y'));
                if (!isNaN(bw) && !isNaN(bh) && !isNaN(bx) && !isNaN(by)) {
                    const ptTopLeft = new SVG.Point(bx, by).transform(shape.matrix());
                    const ptBottomRight = new SVG.Point(bx + bw, by + bh).transform(shape.matrix());
                    sBox = {
                        x: Math.min(ptTopLeft.x, ptBottomRight.x),
                        y: Math.min(ptTopLeft.y, ptBottomRight.y),
                        x2: Math.max(ptTopLeft.x, ptBottomRight.x),
                        y2: Math.max(ptTopLeft.y, ptBottomRight.y),
                        cx: (ptTopLeft.x + ptBottomRight.x) / 2,
                        cy: (ptTopLeft.y + ptBottomRight.y) / 2,
                        width: Math.abs(ptBottomRight.x - ptTopLeft.x),
                        height: Math.abs(ptBottomRight.y - ptTopLeft.y)
                    };
                } else {
                    sBox = shape.bbox().transform(shape.matrix());
                }
            } else {
                sBox = shape.bbox().transform(shape.matrix());
            }
        } else {
            sBox = text.bbox().transform(text.matrix());
        }
        const margin = 5; let anchor = 'middle'; let baseline = 'central';
        // ▼変更: オートフィットで縮小された「正確なフォントサイズ」を最優先で取得する
        let fontSize = NaN;
        if (text.node && text.node.dataset && text.node.dataset.baseFontSize) {
            fontSize = parseFloat(text.node.dataset.baseFontSize);
        }
        if (isNaN(fontSize) || fontSize <= 0) fontSize = parseFloat(text.node.getAttribute('data-base-font-size'));
        ...
        // ▼変更: 要素がない場合にマイナスになるのを防ぐ
        const totalOffset = tspans.length > 0 ? (tspans.length - 1) * lineSpacing : 0;
        ...
        tspans.forEach((node, idx) => {
            ...
            node.removeAttribute('dy'); // ▼ 相対値の 'dy' を完全に廃止
            if (isVertical) {
                tspan.attr({ 'x': targetX - (idx * lineSpacing), 'y': targetY, 'text-anchor': anchor, 'dominant-baseline': baseline });
            } else {
                // ▼ 絶対座標の 'y' を計算して代入し、下にずれていく誤差を根絶する
                tspan.attr({ 'x': targetX, 'y': targetY + (idx * lineSpacing), 'text-anchor': anchor, 'dominant-baseline': baseline });
            }
        });
