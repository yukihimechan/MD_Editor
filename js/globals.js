...
// --- Global State ---
const AppState = {
    documentId: typeof crypto !== 'undefined' && crypto.randomUUID ? crypto.randomUUID() : `doc_${Date.now()}_${Math.floor(Math.random()*10000)}`,
    text: "",
    filePath: null,
    fileHandle: null,
    ...
    config: {
        ...
        previewPastePosition: 'after', // [NEW] プレビューでの貼り付け位置 ('before' または 'after')
        // [NEW] 共同編集設定（フェーズ2: 設定ダイアログから変更可能）
        collab: {
            signalingUrl: '',  // カスタムシグナリングサーバー URL（空の場合はデフォルトを使用）
            turnUrl: '',       // TURN サーバー URL（例: turns:turn.example.com:443）
            turnUsername: '',  // TURN サーバー認証ユーザー名
            turnCredential: '', // TURN サーバー認証パスワード
        }
    },
    viewMode: 'editor-only', // 'split', 'preview-only', 'editor-only'
    // [NEW] 共同編集セッション状態（実行時のみ、永続化しない）
    collab: {
        isActive: false,       // 共同編集セッション中かどうか
        roomName: '',          // 参加中のルーム名
        userName: '',          // 自分の表示名
        userColor: '',         // 自分のカーソルカラー
        connectedUsers: [],    // 接続中の他ユーザー一覧 [{clientId, name, color}]
    },
    searchState: {
        query: "",
        replaceWith: "",
...
const UndoRedoManager = {
    ...
    undo() {
        ...
        // CodeMirror 6 のコマンド関数を使用（CM6 EditorView に undo() メソッドは存在しない）
        if (typeof editorInstance !== 'undefined' && editorInstance && window.CM6 && typeof window.CM6.undo === 'function') {
            window.CM6.undo(editorInstance);
            this.updateButtons();
            return;
        }
    ...
    redo() {
        ...
        // CodeMirror 6 のコマンド関数を使用（CM6 EditorView に redo() メソッドは存在しない）
        if (typeof editorInstance !== 'undefined' && editorInstance && window.CM6 && typeof window.CM6.redo === 'function') {
            window.CM6.redo(editorInstance);
            this.updateButtons();
            return;
        }
