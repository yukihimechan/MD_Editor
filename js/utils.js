...
/**
 * 巨大な画像をリサイズ・圧縮し、軽量なBase64文字列（WebP）を返す
 * 共同編集のWebRTC通信エラー（5MB制限等）を回避するため使用する
 */
    ...
    return new Promise((resolve, reject) => {
        ...
        reader.onload = (e) => {
            ...
            img.onload = () => {
                ...
                // 小さいPNGやGIFは再エンコードによるファイルサイズ増大や透過情報消失を避けるため元のBase64を返す
                if (w <= maxWidth && file.size < 1000000 && isPngOrGif) {
...
async function insertImageAsBase64(file, clientX, clientY, target) {
    try {
        ...
        // [NEW] 共同編集セッション中はリモートピアがローカル画像を参照できないため、
        // 常にBase64埋め込み（document）を強制する
            ...
            if (isSvg) {
                ...
                if (isCollab) {
                    ...
                    try {
                        ...
                        console.warn('[ImageCompress] Compression failed, falling back to original FileReader:', err);
                        const base64 = await fileToDataUrl(file);
                        docContent = `\n\n![${file.name}](${base64})\n\n`;
                        insertToApp(docContent);
                    }
                } else {
                    // 圧縮がOFFかつ単独編集時は、オリジナルの画像をそのままBase64化する
                    try {
                        const base64 = await fileToDataUrl(file);
                        docContent = `\n\n![${file.name}](${base64})\n\n`;
                        insertToApp(docContent);
                    } catch (err) {
                        console.error('[ImageLoad] Raw read failed:', err);
                    }
                }
            }
        }
...
function bindSidebarResize() {
    ...
    // コメントパネル（右側）
    setupResize('comment-panel', '.outline-resize-handle', 'sidebarCommentWidth', true);
}
