...
(function (global) {
    ...
    class ColorPickerUI {
        constructor(optionsOrContainer, options = {}) {
            ...
            this._currentScale = this.options.scale || 1;
        ...
        `;
            ...
            if (this.container) {
                ...
                if (this.options.appendTo) {
                    this.options.appendTo.appendChild(this.wrapper);
                } else {
        ...
        _bindEvents() {
            ...
            if (this.isPopup && this.dom.resizeHandle) {
                ...
                const onPointerMove = (e) => {
                    ...
                    if (this.wrapper) this.wrapper.style.transform = `scale(${this._currentScale})`;
                };

                const onPointerUp = (e) => {
                    document.removeEventListener('pointermove', onPointerMove);
                    document.removeEventListener('pointerup', onPointerUp);
                    this._isResizing = false; // リサイズ状態フラグをリセット
                    this._boundResizeMove = null;
                    this._boundResizeUp = null;
                    if (this.wrapper) this.wrapper.style.transition = ''; // ドラッグ終了後に元に戻す
                };

                // destroy()で解除できるよう参照を保持する
                this._boundResizeMove = onPointerMove;
                this._boundResizeUp = onPointerUp;
                ...
                this.dom.resizeHandle.addEventListener('pointerdown', (e) => {
                    ...
                    this._isResizing = true; // リサイズ中フラグ
            ...
            if (this.isPopup && this.wrapper) {
                ...
                const onDragUp = (e) => {
                    ...
                    this._isDragging = false; // ドラッグ状態フラグをリセット
                    this._boundDragMove = null;
                    this._boundDragUp = null;
                    if (this.wrapper) this.wrapper.classList.remove('cp-dragging');
                };

                // destroy()で解除できるよう参照を保持する
                this._boundDragMove = onDragMove;
                this._boundDragUp = onDragUp;
                ...
                this.wrapper.addEventListener('pointerdown', (e) => {
                    ...
                    this._isDragging = true; // ドラッグ中フラグ
        ...
        updateView(updateInputs = true) {
            if (!this.dom || !this.dom.current) return;
        ...
        show(triggerElement) {
            ...
            if (triggerElement) {
                ...
                let finalTop = Math.max(scrollY, top);
                let finalLeft = Math.max(scrollX, left);

                // appendToが指定されている場合は、親要素からの相対座標に変換する
                if (this.options.appendTo) {
                    const parentRect = this.options.appendTo.getBoundingClientRect();
                    // offsetParentの境界に対しての相対座標とする
                    finalTop = finalTop - (parentRect.top + scrollY);
                    finalLeft = finalLeft - (parentRect.left + scrollX);
                    // dialog等のパディング分が内側に食い込むのを防ぐため簡易補正
                    const parentStyle = window.getComputedStyle(this.options.appendTo);
                    finalTop -= parseFloat(parentStyle.borderTopWidth || 0);
                    finalLeft -= parseFloat(parentStyle.borderLeftWidth || 0);
                }
        ...
        destroy() {
            ...
            // ドラッグ移動中に破棄された場合、document に残った一時リスナーを解除する
            if (this._isDragging && this._boundDragMove) {
                document.removeEventListener('pointermove', this._boundDragMove);
                document.removeEventListener('pointerup', this._boundDragUp);
                this._isDragging = false;
            }
            this._boundDragMove = null;
            this._boundDragUp = null;

            // リサイズ中に破棄された場合、document に残った一時リスナーを解除する
            if (this._isResizing && this._boundResizeMove) {
                document.removeEventListener('pointermove', this._boundResizeMove);
                document.removeEventListener('pointerup', this._boundResizeUp);
