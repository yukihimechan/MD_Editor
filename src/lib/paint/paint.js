...
class PaintLibrary {
  ...
  _createToolbar() {
    ...
    if (this.toolbarItems.clear !== false) {
      const b = this._makeTbBtn('🗑', 'paint-tool-btn', () => { if (confirm('キャンバスをすべて消去しますか？')) this.clearCanvas(); });
  ...
  clearCanvas() { this.ctx.clearRect(0, 0, this.canvas.width, this.canvas.height); this._saveState(); }
  ...
  _createStatusBar() {
    ...
    applyBtn.addEventListener('click', () => {
      ...
      if (confirm(`キャンバスを ${newW} × ${newH} にリサイズしますか？\n現在の内容は消去されます。`)) {
