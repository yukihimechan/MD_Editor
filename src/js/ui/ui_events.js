...
function bindToolbarEvents() {
    ...
    // アノテーション（注釈描画）ボタン
    bindButton('btn-annotation', () => {
        if (typeof window.AnnotationLayer === 'undefined') {
            console.warn('[Annotation] AnnotationLayer が読み込まれていません。');
            return;
        }
        const isNowActive = window.AnnotationLayer.toggle();
        const btn = document.getElementById('btn-annotation');
        if (btn) {
            btn.classList.toggle('annotation-mode-on', isNowActive);
            btn.title = isNowActive ? 'アノテーション終了 (注釈描画を終了)' : 'アノテーション（注釈描画）';
        }
    });
}
