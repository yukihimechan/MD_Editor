...
function toggleOutline() {
    ...
    if (willBeVisible) {
        ...
        // パネルを開くときは常に最新状態で再構築する（キャッシュリセット）
        buildOutline._lastSig = null;
        buildOutline();
    } else {
 ...
 * 見出し一覧が前回と変わっていない場合は再構築をスキップする
 */
function buildOutline() {
    const headings = DOM.preview.querySelectorAll('h1, h2, h3, h4, h5, h6');

    // --- ハッシュ比較によるスキップ ---
    // 見出しの「レベル・テキスト・ID・折りたたみ状態」を連結してシグネチャを生成する。
    // 前回と同一なら DOM 書き換えとイベント再バインドをスキップしてパフォーマンスを確保する。
    let sig = '';
    headings.forEach(h => {
        const level   = h.tagName;           // H1〜H6
        const text    = h.textContent.trim();
        const id      = h.id || '';
        const folded  = h.classList.contains('collapsed') ? '1' : '0';
        sig += `${level}|${text}|${id}|${folded};`;
    });
    if (buildOutline._lastSig === sig) {
        // 変化なし：アクティブ行ハイライトだけ更新して終了
        if (typeof updateActiveOutlineItem === 'function') updateActiveOutlineItem();
        return;
    }
    buildOutline._lastSig = sig;
    // --- ここまでスキップ判定 ---
...
function initContextMenu() {
    ...
    const menuStructure = [
        ...
        { label: t('contextMenuEditor.copy') || 'コピー', action: 'copy', id: 'menu-copy', shortcut: 'Ctrl+C' },
        {
            ...
            id: 'menu-insert',
            children: [
                { label: t('contextMenuEditor.text') || 'テキスト (段落)', action: 'insert-text-block', id: 'menu-insert-text' },
                {
                    label: t('contextMenuEditor.heading') || '見出し',
                    children: [
                        { label: t('contextMenuEditor.heading1') || '見出し1', action: 'insert', value: '# ', type: 'heading' },
                        { label: t('contextMenuEditor.heading2') || '見出し2', action: 'insert', value: '## ', type: 'heading' },
                        { label: t('contextMenuEditor.heading3') || '見出し3', action: 'insert', value: '### ', type: 'heading' },
                        { label: t('contextMenuEditor.heading4') || '見出し4', action: 'insert', value: '#### ', type: 'heading' },
                        { label: t('contextMenuEditor.heading5') || '見出し5', action: 'insert', value: '##### ', type: 'heading' },
                        { label: t('contextMenuEditor.heading6') || '見出し6', action: 'insert', value: '###### ', type: 'heading' }
                    ]
                },
                {
                ...
                },
                {
                    ...
                    children: [
                        { label: t('contextMenuEditor.fileLink') || 'ファイル', action: 'insert', value: '[リンクテキスト](filename.md)', type: 'inline' },
                        { label: t('contextMenuEditor.imageLink') || '画像', action: 'insert', value: '![画像タイトル](image.png)', type: 'inline' }
                    ]
                },
                { label: t('contextMenuEditor.codeBlock') || 'コードブロック', action: 'insert', value: '```\n\n```', type: 'block' },
                {
                    ...
                    children: [
                        { label: t('contextMenuEditor.table2Col') || '2列', action: 'insert', value: '\n\n| Header 1 | Header 2 |\n| --- | --- |\n| Cell 1 | Cell 2 |', type: 'table' },
                        { label: t('contextMenuEditor.table3Col') || '3列', action: 'insert', value: '\n\n| Header 1 | Header 2 | Header 3 |\n| --- | --- | --- |\n| Cell 1 | Cell 2 | Cell 3 |', type: 'table' },
                        { label: t('contextMenuEditor.table4Col') || '4列', action: 'insert', value: '\n\n| Header 1 | Header 2 | Header 3 | Header 4 |\n| --- | --- | --- | --- |\n| Cell 1 | Cell 2 | Cell 3 | Cell 4 |', type: 'table' }
                    ]
                },
                {
                    label: t('contextMenuEditor.taskList') || 'タスクリスト',
                    children: [
                        { label: t('contextMenuEditor.taskChecked') || 'チェック有', action: 'insert', value: '- [x] ', type: 'list' },
                        { label: t('contextMenuEditor.taskUnchecked') || 'チェック無', action: 'insert', value: '- [ ] ', type: 'list' }
                    ]
                },
        ...
        { type: 'separator' },
        { label: t('contextMenuEditor.deleteObject') || '削除', action: 'delete', id: 'menu-delete' }
    ];
    ...
    function adjustSubmenuPosition(submenu) {
        ...
        // 前回表示時の位置調整（インラインスタイル）が残っていると誤計算の原因になるためリセット
    ...
    if (!_contextMenuEventsBound) {
        document.addEventListener('contextmenu', (e) => {
            ...
            if (isInPreview) {
                ...
                // 右クリック時にも、左クリックと同様に指定要素へ青枠（フォーカス）を移動させる
                if (isPreviewSelectable && typeof PreviewInlineEdit !== 'undefined' && typeof PreviewInlineEdit.startFocus === 'function') {
                    // クリック不可能なアクションボタン等は除外する
                    const isExcludedFocus = e.target.closest('a[href], button, input, .drag-handle, .img-resize-wrapper');
                                         
                    if (!isExcludedFocus) {
                        // 既に選択群に含まれていなければ、単一選択で再フォーカスする
                        const isSelected = PreviewInlineEdit.selectedElements instanceof Set 
                                            ? PreviewInlineEdit.selectedElements.has(isPreviewSelectable)
                                            : PreviewInlineEdit.focusedElement === isPreviewSelectable;
                        
                        if (!isSelected) {
                            PreviewInlineEdit.startFocus(isPreviewSelectable, true, false);
                        }
                    }
                }
            }
...
function handleMenuAction(item, target) {
    ...
    if (item.action === 'insert') {
        ...
        if (target.closest('.md-preview') && typeof PreviewInlineEdit !== 'undefined' && PreviewInlineEdit.isFocused && !PreviewInlineEdit.isEditing && typeof PreviewInlineEdit._handleFocusModeInsert === 'function') {
            // フォーカスモードからの挿入：スラッシュコマンドと同様の「挿入後に編集状態にする」フローを呼び出す
            const block = target.closest('.preview-selectable') || target;
            PreviewInlineEdit._focusModeInsertSource = block;
            PreviewInlineEdit._handleFocusModeInsert(textToInsert, item.action, item);
        } else {
            performInsert(textToInsert, target);
        }
    }
    else if (item.action === 'insert-text-block') {
        if (target.closest('.cm-editor') && window.editorInstance) {
            const view = window.editorInstance;
            const selection = view.state.selection.main;
            const content = '\n\n';
            view.dispatch({
            ...
            if (typeof PreviewInlineEdit !== 'undefined' && typeof PreviewInlineEdit.insertTextBlock === 'function') {
                const block = target.closest('.preview-selectable') || target;
                PreviewInlineEdit.insertTextBlock(block);
            }
        }
    }
        ...
        if (target.closest('.cm-content')) {
            ...
            if (selection && selection.toString()) {
                ...
                // 文字列が選択されていない場合は、青枠で囲われたブロックのMarkdownソースをコピー
                if (typeof PreviewInlineEdit !== 'undefined' && typeof PreviewInlineEdit.copySelectedBlocks === 'function') {
                    PreviewInlineEdit.copySelectedBlocks();
                }
            }
        }
    }
        ...
        if (target.closest('.cm-editor')) {
            ...
            if (window.editorInstance) {
                ...
                navigator.clipboard.readText().then(text => {
                    ...
                    showToast(t('toast.clipboardReadError'), 'error');
                });
            }
        } else if (target.closest('.md-preview')) {
            // [NEW] プレビューでの貼り付け
            if (typeof PreviewInlineEdit !== 'undefined' && typeof PreviewInlineEdit.pasteBlocks === 'function') {
                PreviewInlineEdit.pasteBlocks();
            }
        }
    }
        ...
        if (typeof PreviewInlineEdit !== 'undefined' && typeof PreviewInlineEdit.deleteBlock === 'function') {
            // [CHANGE] まず青枠でフォーカスされている要素を明確な対象として選択する
            const block = (PreviewInlineEdit.focusedElement) || target.closest('.preview-selectable') || target;
            PreviewInlineEdit.deleteBlock(block);
            
            // 削除後、存在しない要素へのフォーカス残りによるバグを防ぐため青枠をクリア
            if (typeof PreviewInlineEdit.clearFocus === 'function') {
        ...
        if (headings.length === 0) {
            if (typeof showToast === 'function') showToast(t('error.noHeadingsForToc'), "error");
            return;
        }
        ...
        if (headings.length === 0 && changes.length === 0) {
            if (typeof showToast === 'function') showToast(t('error.noHeadingFound'), "error");
            return;
        }
    ...
    DOM.preview.addEventListener('click', (e) => {
        ...
        if (AppState.config && AppState.config.replaceUnicodeCheckboxes) {
            ...
            if (range && range.startContainer && range.startContainer.nodeType === Node.TEXT_NODE) {
                ...
                // Check if the click offset is exactly on or right after the checkbox character
                if (offset < text.length && checkboxes.includes(text[offset])) {
                    targetChar = text[offset];
                    targetOffset = offset;
                } else if (offset > 0 && checkboxes.includes(text[offset - 1])) {
                    targetChar = text[offset - 1];
                    ...
                    if (blockEl) {
                        ...
                        while ((currentNode = walker.nextNode())) {
                            if (currentNode === range.startContainer) {
                                for (let i = 0; i < targetOffset; i++) {
                                    if (checkboxes.includes(currentNode.textContent[i])) nth++;
                                }
                                found = true;
                                break;
                            } else {
                        ...
                        if (found) {
                            ...
                            if (!isNaN(lineNum)) {
                                ...
                                else if (targetChar === '☑') replacement = '☒';
                                else if (targetChar === '☒') replacement = '☐';
                                ...
                                // Sometimes data-line-end is exclusive, but to be safe we check up to lineEndNum
                                const endIdx = Math.min(lineEndNum, lines.length - 1);
                                ...
                                for (let i = startIdx; i <= endIdx; i++) {
                                    ...
                                    // Find the occurrences in this line
                                    while ((match = regex.exec(lineText)) !== null) {
                                        if (count === nth) {
                                            foundLineIdx = i;
                                            foundMatchIndex = match.index;
                                            break;
                                        }
                                        count++;
                                    }
                                ...
                                if (foundLineIdx !== -1) {
                                    ...
                                    const newLineText = lineText.substring(0, foundMatchIndex) + replacement + lineText.substring(foundMatchIndex + 1);
                                    
                                    if (typeof editorView !== 'undefined') {
                                        const lineObj = editorView.state.doc.line(foundLineIdx + 1);
                                        const from = lineObj.from;
                                        const to = lineObj.to;
                                        ...
                                        lines[foundLineIdx] = newLineText;
                                        setEditorText(lines.join('\n'));
                                        AppState.text = getEditorText();
                                        AppState.isModified = true;
                                        if (typeof render === 'function') render();
...
/**
 * Initialize Table Cell Copy functionality
 */
function initTableCellCopy() {
    if (!DOM.preview) return;

    const copyBtn = document.createElement('button');
    copyBtn.className = 'table-cell-copy-btn';
    copyBtn.dataset.i18n = "code.copyButton";
    copyBtn.textContent = typeof I18n !== 'undefined' ? I18n.translate("code.copyButton") : "コピー";
    ...
    copyBtn.addEventListener('click', (e) => {
        ...
        if (copyBtn.parentElement) {
            // Get text content of the cell, excluding the button's own text
            const clone = copyBtn.parentElement.cloneNode(true);
            const btnInClone = clone.querySelector('.table-cell-copy-btn');
    ...
    DOM.preview.addEventListener('mouseout', (e) => {
        // If we leave the table entirely, remove the button
        if (!e.relatedTarget || !e.relatedTarget.closest('td, th')) {
            if (copyBtn.parentElement) {
...
function createOutlineMenu() {
    ...
    if (AppState.isSvgMode) {
        ...
        outlineMenuDropdown.innerHTML = `
            ...
            <div class="file-menu-item" data-action="comment">
                <span data-i18n="outlineMenu.comment">コメント一覧</span>
...
function toggleFileMenu(e) {
    ...
    const isCurrentlyVisible = fileMenuDropdown && fileMenuDropdown.classList.contains('visible');
...
function createFileMenu() {
    ...
    if (typeof window.autoSaveManager !== 'undefined') {
        const history = window.autoSaveManager.getHistory();
        if (history && history.length > 0) {
            let historyHtml = '<div class="file-menu-separator"></div>';
            historyHtml += '<div style="padding: 4px 12px; font-size: 0.8em; color: #888; pointer-events: none;">一時保存履歴</div>';
            
            history.forEach(item => {
                const dateStr = window.autoSaveManager.formatDate(item.timestamp);
                const name = item.filePath || '新規ドキュメント';
                historyHtml += `
                    <div class="file-menu-item history-item" data-history-id="${item.id}" title="${name}&#10;${dateStr}">
                        <span style="overflow: hidden; text-overflow: ellipsis; white-space: nowrap; flex: 1;">${name} - <span style="font-size: 0.9em; color: #666;">${dateStr}</span></span>
                    </div>
                `;
            });
            
            fileMenuDropdown.innerHTML += historyHtml;
        }
    }
    ...
    // メニュー項目のクリックイベント
    fileMenuDropdown.querySelectorAll('.file-menu-item:not(.history-item)').forEach(item => {
        item.onclick = () => {
            const action = item.dataset.action;
    ...
    // 履歴項目のクリックイベント
    fileMenuDropdown.querySelectorAll('.history-item').forEach(item => {
        item.onclick = () => {
            const id = item.dataset.historyId;
            if (typeof window.autoSaveManager !== 'undefined') {
                window.autoSaveManager.loadFromHistory(id);
            }
            closeFileMenu();
        };
    });
...
function createExportMenu() {
    ...
    exportMenuDropdown.innerHTML = `
        ...
        <div class="export-menu-item" data-action="export-standalone">
            <svg class="icon" width="16" height="16"><use href="#icon-export" /></svg>
            <span data-i18n="exportMenu.standalone">HTML (エディタ同梱)</span>
        </div>
...
function handleExportAction(action) {
    switch (action) {
        ...
        case 'export-standalone':
            if (typeof ExportStandalone !== 'undefined') ExportStandalone.run();
            break;
    }
}
