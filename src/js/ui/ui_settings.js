...
function openConfig() {
    ...
    // [NEW] Unicode checkboxes replace
    const replaceCheckboxesCheck = document.getElementById('cfg-replace-unicode-checkboxes');
    if (replaceCheckboxesCheck) {
        replaceCheckboxesCheck.checked = AppState.config.replaceUnicodeCheckboxes || false;
    }

    // [NEW] プレビューでの貼り付け位置
    const pastePositionSelect = document.getElementById('cfg-preview-paste-position');
    if (pastePositionSelect) {
        pastePositionSelect.value = AppState.config.previewPastePosition || 'after';
    }
    ...
    const initColorButton = (id, colorVal) => {
        ...
        if (!btn._picker && typeof ColorPickerUI !== 'undefined') {
            btn._picker = new ColorPickerUI({
                ...
                isPopup: true,
                layout: 'vertical',
                appendTo: document.getElementById('dialog-config'),
                onChange: (colorObj) => {
    ...
    // [NEW] Compress Standalone Image
    const compressStandaloneInput = document.getElementById('cfg-compress-standalone-image');
    if (compressStandaloneInput) {
        compressStandaloneInput.checked = AppState.config.compressStandaloneImage !== false;
    }

    // [NEW] 共同編集設定（フェーズ2）の読み込み
    const collabCfg = AppState.config.collab || {};
    const collabSignalingEl = document.getElementById('cfg-collab-signaling-url');
    const collabTurnUrlEl = document.getElementById('cfg-collab-turn-url');
    const collabTurnUserEl = document.getElementById('cfg-collab-turn-username');
    const collabTurnCredEl = document.getElementById('cfg-collab-turn-credential');
    if (collabSignalingEl) collabSignalingEl.value = collabCfg.signalingUrl || '';
    if (collabTurnUrlEl)   collabTurnUrlEl.value   = collabCfg.turnUrl || '';
    if (collabTurnUserEl)  collabTurnUserEl.value  = collabCfg.turnUsername || '';
    if (collabTurnCredEl)  collabTurnCredEl.value  = collabCfg.turnCredential || '';
...
async function saveConfig() {
    ...
    // [NEW] Unicode checkboxes replace
    const replaceCheckboxesInput = document.getElementById('cfg-replace-unicode-checkboxes');
    if (replaceCheckboxesInput) {
        AppState.config.replaceUnicodeCheckboxes = replaceCheckboxesInput.checked;
    }

    // [NEW] プレビューでの貼り付け位置
    const pastePositionInput = document.getElementById('cfg-preview-paste-position');
    if (pastePositionInput) {
        AppState.config.previewPastePosition = pastePositionInput.value;
    }
    ...
    const compressStandaloneInput = document.getElementById('cfg-compress-standalone-image');
    if (compressStandaloneInput) {
        AppState.config.compressStandaloneImage = compressStandaloneInput.checked;
    }

    // [NEW] 共同編集設定（フェーズ2）
    const collabSignalingInput = document.getElementById('cfg-collab-signaling-url');
    const collabTurnUrlInput = document.getElementById('cfg-collab-turn-url');
    const collabTurnUserInput = document.getElementById('cfg-collab-turn-username');
    const collabTurnCredInput = document.getElementById('cfg-collab-turn-credential');
    if (!AppState.config.collab) AppState.config.collab = {};
    if (collabSignalingInput) AppState.config.collab.signalingUrl = collabSignalingInput.value.trim();
    if (collabTurnUrlInput)   AppState.config.collab.turnUrl = collabTurnUrlInput.value.trim();
    if (collabTurnUserInput)  AppState.config.collab.turnUsername = collabTurnUserInput.value.trim();
    if (collabTurnCredInput)  AppState.config.collab.turnCredential = collabTurnCredInput.value.trim();
...
function updateAllSVGWidthsInEditor() {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (inSvgBlock && line.includes('<svg')) {
            const result = normalizeSVGTagForPreviewWidth(line, targetWidth);
            if (result.changed) {
                lines[i] = result.line;
                hasChanges = true;
            }
    ...
    if (hasChanges) {
        ...
        // グローバルの setEditorText または DOM.editorInstance を使用する
...
/**
 * [NEW] SVGタグ行を previewWidth に合わせて正規化する共通ヘルパー。
 * 3つのケースを処理する:
 *   ケース1: アプリ作成SVG（data-paper-width あり）
 *             → width/height（表示サイズ）のみを更新。data-paper-* は変更しない。
 *   ケース2: 外部SVG（data-paper-width なし、viewBox あり）
 *             → viewBox の比率を保ったまま width/height を更新し、data-paper-* を追記する。
 *   ケース3: 外部SVG（data-paper-width なし、viewBox なし）
 *             → width/height 属性から viewBox を自動生成し、data-paper-* を追記する。
 *
 * @param {string} sourceLine   - `<svg ...>` タグを含む1行の文字列
 * @param {number} targetWidth  - 新しいプレビュー横幅（configWidth）
 * @returns {{ changed: boolean, line: string }} 変更の有無と更新後の行文字列
 */
function normalizeSVGTagForPreviewWidth(sourceLine, targetWidth) {
    const svgMatch = sourceLine.match(/<svg[^>]*>/);
    if (!svgMatch) return { changed: false, line: sourceLine };
    ...
    const dpwMatch  = svgTag.match(/data-paper-width="([^"]+)"/);
    const dphMatch  = svgTag.match(/data-paper-height="([^"]+)"/);

    // --- ケース1: アプリ作成SVG（data-paper-width あり） ---
    if (dpwMatch && dphMatch) {
        const paperW = parseFloat(dpwMatch[1]);
        const paperH = parseFloat(dphMatch[1]);
        if (!paperW || !paperH) return { changed: false, line: sourceLine };

        const newW = targetWidth;
        const newH = Math.round(targetWidth * (paperH / paperW));
        const currentW = wMatch ? parseFloat(wMatch[1]) : 0;
        if (currentW === newW) return { changed: false, line: sourceLine };

        // width / height のみ更新（data-paper-* は変えない）
        svgTag = wMatch
            ? svgTag.replace(/width="[^"]+"/, `width="${newW}"`)
            : svgTag.replace('<svg', `<svg width="${newW}"`);
        svgTag = hMatch
            ? svgTag.replace(/height="[^"]+"/, `height="${newH}"`)
            : svgTag.replace('<svg', `<svg height="${newH}"`);

        console.log(`[normalizeSVG] ケース1: width ${currentW} -> ${newW}, height -> ${newH}`);
        return { changed: svgTag !== originalTag, line: sourceLine.replace(originalTag, svgTag) };
    }

    // --- ケース2: 外部SVG（viewBox あり） ---
    if (vbMatch) {
        const vb = vbMatch[1].split(/[\s,]+/).map(parseFloat);
        if (vb.length !== 4 || isNaN(vb[2]) || vb[2] <= 0 || isNaN(vb[3]) || vb[3] <= 0) {
            return { changed: false, line: sourceLine };
        }
        const [vx, vy, vw, vh] = vb;
        const newW = targetWidth;
        const newH = Math.round(targetWidth * (vh / vw));
        ...
        // data-paper-* を追記（エディタが論理サイズを扱えるようにする）
        const paperAttrs = `data-paper-width="${vw}" data-paper-height="${vh}" data-paper-x="${vx}" data-paper-y="${vy}" data-paper-zoom="100" data-paper-offx="0" data-paper-offy="0"`;
        svgTag = svgTag.replace(/>$/, ` ${paperAttrs}>`);

        console.log(`[normalizeSVG] ケース2: viewBox=${vbMatch[1]}, width->${newW}, height->${newH}, data-paper追記`);
        return { changed: svgTag !== originalTag, line: sourceLine.replace(originalTag, svgTag) };
    }

    // --- ケース3: 外部SVG（viewBox なし） ---
    {
        const logicalW = (wMatch && parseFloat(wMatch[1]) > 0) ? parseFloat(wMatch[1]) : targetWidth;
        const logicalH = (hMatch && parseFloat(hMatch[1]) > 0) ? parseFloat(hMatch[1]) : 600;
        const newW = targetWidth;
        const newH = Math.round(targetWidth * (logicalH / logicalW));

        // viewBox を追加
        svgTag = svgTag.replace('<svg', `<svg viewBox="0 0 ${logicalW} ${logicalH}"`);
        ...
        // data-paper-* を追記
        const paperAttrs = `data-paper-width="${logicalW}" data-paper-height="${logicalH}" data-paper-x="0" data-paper-y="0" data-paper-zoom="100" data-paper-offx="0" data-paper-offy="0"`;
        svgTag = svgTag.replace(/>$/, ` ${paperAttrs}>`);

        console.log(`[normalizeSVG] ケース3: viewBox生成 0 0 ${logicalW} ${logicalH}, width->${newW}, height->${newH}`);
        return { changed: svgTag !== originalTag, line: sourceLine.replace(originalTag, svgTag) };
    }
}
window.normalizeSVGTagForPreviewWidth = normalizeSVGTagForPreviewWidth;
...
function applySidebarWidths() {
    ...
    const commentWidth = AppState.config.sidebarCommentWidth || 300;
    ...
    document.documentElement.style.setProperty('--sidebar-comment-width', `${commentWidth}px`);
}
...
function loadSettings() {
    try {
        ...
        if (s) {
            ...
            // [NEW] Compress Standalone Image
            if (AppState.config.compressStandaloneImage === undefined) {
                AppState.config.compressStandaloneImage = true;
            }
            ...
            if (AppState.config.sidebarCommentWidth === undefined) {
                AppState.config.sidebarCommentWidth = 300;
            }
            ...
            // [NEW] Unicode checkboxes replace
            if (AppState.config.replaceUnicodeCheckboxes === undefined) {
                AppState.config.replaceUnicodeCheckboxes = false;
            }

            // [NEW] 共同編集設定のデフォルト値
            if (!AppState.config.collab) {
                AppState.config.collab = {
                    signalingUrl: '',
                    turnUrl: '',
                    turnUsername: '',
                    turnCredential: '',
