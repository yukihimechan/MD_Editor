    ...
    bindEvents() {
        ...
        document.addEventListener('keydown', (e) => {
            ...
            if (isInputTarget && !this.isEditing && this.isFocused && this.focusedElement && this._logicalFocus === 'preview') {
                if ((e.key === 'Delete' || e.key === 'Backspace') &&
                    !window.currentEditingSVG && !(typeof window.isTableEditing === 'function' ? window.isTableEditing() : window.isTableEditing) &&
            ...
            if (!this.isEditing && this.isFocused && this.focusedElement) {
                ...
                if (e.key === ' ' && this.focusedElement.tagName.match(/^H[1-6]$/i)) {
                    ...
                    // [NEW] SVGエディタ・表エディタが起動中の時は、内部での図形/セルなどの削除として扱うため、ブロック丸ごとの削除はスキップする
                    if (window.currentEditingSVG || (typeof window.isTableEditing === 'function' ? window.isTableEditing() : window.isTableEditing)) {
                        return;
                    }
