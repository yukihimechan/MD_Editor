...
const PreviewInlineEdit = {
    ...
    restoreFocusIfNeeded() {
        ...
        if (targetLine !== null && !this.isEditing) {
            ...
            const focusableSelectors = 'h1, h2, h3, h4, h5, h6, p, li, blockquote, .dummy-tail-block, .image-caption'.split(',').map(s => s.trim());
            ...
            if (this.pendingActionType) {
                const { type, isBlockLevel, startLineIndex, newBlockText } = this.pendingActionType;
                if (!isBlockLevel) {
    ...
    startFocus(element, syncToEditor = true) {
        ...
        this.focusedLineIndex = parseInt(element.getAttribute('data-line')); // [FIX] フォーカスされた行を永続記録
    ...
    getFocusableElements(headingsOnly = false) {
        const selectors = headingsOnly 
            ? 'h1, h2, h3, h4, h5, h6' 
            : 'h1, h2, h3, h4, h5, h6, p, li, blockquote, .dummy-tail-block, .image-caption';
    ...
    moveFocus(direction, headingsOnly = false) {
        ...
        const currentIndex = allElements.indexOf(this.focusedElement);
    ...
    getSelectableTarget(element) {
        ...
        // Excluded tables (TD, TH) as they have their own editor
    ...
    startEditing(element, initialChar = '', clientX = null, clientY = null, selectAll = false) {
        ...
        if (!this.isDummyBlock) {
            ...
            this.currentSourceInfo = this.findSourceLocation(currentText, element.tagName, element);
        ...
        if (element.tagName === 'LI' && element.classList.contains('task-list-item')) {
            const checkbox = element.querySelector('input[type="checkbox"]');
            if (checkbox) {
                let wrapper = element.querySelector('.task-list-item-text-wrapper');
                if (!wrapper) {
                ...
                this._taskListOriginalDisplay = element.style.display || '';
    ...
    handleKeydown(e) {
        ...
        if (e.key === 'Escape') {
            ...
            e.stopPropagation();
            this.saveEditing();
        } else if (e.key === 'Enter' && !e.shiftKey && this.selectedElement.tagName === 'LI') {
            // [NEW] リスト項目の場合の Enter キー拡張挙動
            e.preventDefault();
            e.stopPropagation();
            
            // 内容が空かどうかをチェック
            const content = this.selectedElement.textContent.trim();
            if (content === '') {
                // 空ならリストから離脱
    ...
    async saveEditing() {
        ...
        if (!this.currentSourceInfo) {
            ...
            const updatedFullText = fullText + appendedText;
            
            window.isScrolling = true;
            if (typeof window.updateEditorRange === 'function') {
                window.updateEditorRange(fullText.length, fullText.length, appendedText);
            } else {
        ...
        if (tagName.match(/^H[1-6]$/)) {
            ...
            // 引用 (例: "> 引用" -> match[0] = "> ")
            const match = originalSourceText.match(/^(>\s*)+/);
            if (match) newText = match[0] + newText;
        } else if (tagName === 'LI') {
            ...
            if (this.listAction === 'add_next') {
                ...
                // リストを抜けるため、マーカーを削除した状態（ただの改行2つ）にする
                newText = "\n\n"; 
            } else {
                newText = prefix + newText;
            }
        ...
        if (nextEditLiIndex !== -1) {
            ...
            if (targetLi) {
                // 古いLIからフォーカス表示を外し、新しいLIにフォーカスを移す
                this.startFocus(targetLi, false);
                // 座標情報をダミーで渡すか、引数なしで呼べば要素の先頭/末尾にキャレットが合う
                this.startEditing(targetLi);
            }
        }
        
        // [NEW] 矢印キーやTabキーでのナビゲーション時の次の要素へのフォーカス移行
    ...
    async _handleFocusModeInsert(contentToInsert, actionType, item) {
        ...
        const needsEditing = item && (item.type === 'heading' || item.type === 'list');
        if (needsEditing) {
    ...
    findSourceLocation(previewText, tagName, element) {
        ...
        // Strategy 0: Use data-line attribute (MOST ACCURATE)
        const dataLine = element ? (element.getAttribute('data-line') || element.closest('[data-line]')?.getAttribute('data-line')) : null;
        if (dataLine) {
            const lineNum = parseInt(dataLine);
            const lines = fullText.split('\n');
            if (lineNum > 0 && lineNum <= lines.length) {
                // Find start position of the line
                let currentPos = 0;
                ...
                let end = currentPos + lines[lineNum - 1].length;

                // Expand to the full block if it spans multiple lines (markdown-it provides end line)
                const dataLineEndAttr = element.getAttribute('data-line-end') || element.closest('[data-line-end]')?.getAttribute('data-line-end');
                if (dataLineEndAttr) {
                    const endLineNum = parseInt(dataLineEndAttr);
                    if (endLineNum > lineNum) {
                        let endPos = currentPos;
                        for (let i = lineNum - 1; i < endLineNum; i++) {
                            if (i < lines.length) {
                                // Add newline only if not the last line of this block
                                const isLastLineOfBlock = (i === endLineNum - 1);
                                endPos += lines[i].length + (isLastLineOfBlock ? 0 : 1);
                            }
                        }
                        end = endPos;
                    }
                ...
                start = Math.min(start, fullText.length);
        ...
        // Strategy 1: Exact search (if no formatting)
        let index = fullText.indexOf(previewText);

        // Strategy 2: If simple search fails, try to account for Markdown characters
        // This is complex. For now, let's implement a heuristic:
        // Search for a block that *contains* the preview text characters in sequence?
        ...
        if (index === -1) {
            ...
            if (tagName && tagName.startsWith('H')) {
                ...
                // Search for line starting with hashes and containing text
                ...
                for (let i = 0; i < lines.length; i++) {
                    const line = lines[i];
                    // Very rough check: line starts with hashes, and contains most of the text?
                    if (line.trim().startsWith(hashes)) {
                        // Check content match
                        const lineContent = line.replace(/^#+\s*/, '');
                        // If lineContent roughly matches cleanPreview (ignoring markdown symbols like **, *)
                        const cleanLine = lineContent.replace(/[*_~`]/g, '');
                        if (cleanLine.includes(cleanPreview) || cleanPreview.includes(cleanLine)) {
                            return {
                                start: currentPos,
            ...
            // Split full text by blank lines to get "blocks"
            // Find a block that matches the logic
            // This is actually surprisingly hard to do perfectly without a source map.

            // [FIX] Strategy 2.5: Fuzzy search (ignoring Markdown symbols like **, _, ~, `)
            const escapeRegExp = (string) => string.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');

            // Construct a regex that allows optional Markdown characters between characters of the preview text.
            // This allows matching "**text**" even if we only have "text".
            const snippet = previewText.trim().substring(0, 15);
            if (snippet.length >= 3) {
                // Allow optional leading symbols/whitespace
                let pattern = "[\\*\\_\\~\\`\\#\\s]*";
                for (let i = 0; i < snippet.length; i++) {
                    ...
                    if (/\s/.test(char)) {
                        ...
                        // Match the character and allow any number of Markdown symbols right after it
                ...
                try {
                    ...
                    if (match) {
                        index = match.index;
                        console.log(`[PreviewInlineEdit] Fuzzy match successful at index ${index} for snippet: ${snippet}`);
                    }
                } catch (e) {
                    console.warn('[PreviewInlineEdit] Fuzzy regex failed:', e);
                }
            }

            // Fallback: Simple 'includes' search but ignoring leading non-active chars
            if (index === -1) {
                const simpleSnippet = previewText.substring(0, 20).trim();
                if (simpleSnippet) {
                    index = fullText.indexOf(simpleSnippet);
                }
            }
        }
        if (index !== -1) {
            // Determine expansion mode based on tag
            // Line-based (Stop at single newline): LI, H1-H6, TD, TH
            // Block-based (Stop at double newline): P, BLOCKQUOTE
            const isBlockTag = ['P', 'BLOCKQUOTE'].includes(tagName);

            let start = index;
            let end = index + previewText.length;

            if (isBlockTag) {
                // Block Mode: Expand to double newline
                while (start > 0) {
                    if (start >= 2 && fullText.substring(start - 2, start) === '\n\n') break;
                    if (start === 0) break;
                    start--;
                }
                ...
                while (end < fullText.length) {
                    if (fullText.substring(end, end + 2) === '\n\n') break;
                    end++;
                }
            } else {
                // Line Mode: Expand to single newline (start of line / end of line)
                while (start > 0) {
                    if (fullText[start - 1] === '\n') break;
                    start--;
                }
                ...
                // Special check for LI: multiline list items? 
                // For now, strict line mode is requested by user ("I was surprised it opened the whole list").
                // If we want to support multiline LI, we'd need to check indentation, which is complex.
                // Sticking to single line for LI matches the "highlighted border" visual.
            }
