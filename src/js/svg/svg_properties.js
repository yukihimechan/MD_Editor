...
function showResizeHandle(container) {
    ...
    if (!resizer) {
        ...
        resizer.onmousedown = (e) => {
            ...
            // [FIX] startHeightは「論理サイズ (baseHeight)」でなく
            // 「DOMの表示高さ (height属性)」を基準にする。
            // これにより configWidth ≠ data-paper-width の場合でた startHeight=600 で
            // 高さが来るジャンプを防ぐ。
            const svgNode = cur.draw ? cur.draw.node : null;
            const startDisplayH = svgNode
                ? (parseFloat(svgNode.getAttribute('height')) || cur.baseHeight || 450)
                : (cur.baseHeight || 450);

            // [FIX] display座標→logical座標の変換比率
            // baseWidth (logical) / displayWidth (configWidth)
            const configW = (typeof AppState !== 'undefined' && AppState.config && AppState.config.previewWidth)
                ? AppState.config.previewWidth : (cur.baseWidth || 820);
            const displayToLogical = (cur.baseWidth || configW) / configW;

            // ドラッグ開始時の論理高さ（固定値）
            const startLogicalH = cur.baseHeight || 450;

            const onMouseMove = (moveEvent) => {
                const deltaDisplayY = moveEvent.clientY - startY;
                // displayピクセルの移動量を logical 単位に変換
                const deltaLogicalY = deltaDisplayY * displayToLogical;
                const newLogicalH = Math.max(50, startLogicalH + deltaLogicalY);
                updateSVGHeight(newLogicalH);
 ...
 * newHeight: 論理キャンバスの高さ (data-paper-height 単位)
 */
function updateSVGHeight(newHeight) {
    ...
    // [FIX] configWidth に合わせた表示高を計算し、width/height を同時に更新する。
    // これをしないと viewBox と表示サイズの縦横比がずれて
    // グリッドが左右に消える (letterbox) バグが発生する。
    const configW = (typeof AppState !== 'undefined' && AppState.config && AppState.config.previewWidth)
        ? AppState.config.previewWidth : (current.baseWidth || 820);
    const displayH = (configW !== current.baseWidth && current.baseWidth)
        ? Math.round(newHeight * configW / current.baseWidth)
        : Math.round(newHeight);

    svgNode.setAttribute('width', configW);
    svgNode.setAttribute('height', displayH);
    svgNode.setAttribute('data-paper-height', Math.round(newHeight));
