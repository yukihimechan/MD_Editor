...
const SVGPathOps = {
                ...
                try {
                    ...
                    reader.onload = (event) => {
                        try {
                            ...
                            alert(`フォントの解析に失敗しました: ${file.name}\n${pe.message}`);
                            resolve(null);
                        }
                    };
    ...
    togglePathCutMode(btn) {
        ...
        if (!draw) {
            if (window.showToast) window.showToast("SVGキャンバスが見つかりません。");
            this.stopPathCutMode();
            return;
        }
        ...
        if (window.showToast) window.showToast("切断したいパスの上にマウスを合わせてクリックしてください。");
        ...
        if (this._currentHit) {
            ...
            try {
                ...
                if (location.offset < margin || location.offset > targetPath.length - margin) {
                    if (window.showToast) window.showToast("パスの端すぎるため切断できません。");
                    return; // abort
                }
                ...
                if (!parts && !wasClosed) {
                    if (window.showToast) window.showToast("指定の位置でパスを分割できませんでした。");
                    return;
                }
                ...
                if (window.showToast) window.showToast("パスを切断しました。");
