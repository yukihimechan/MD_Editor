...
const SvgTextEditor = {
    ...
    saveAndClose: function (skipSelect = false) {
        ...
        // ▼変更: オートフィットで縮小された「正確なフォントサイズ」を最優先で取得する
        let finalFontSize = NaN;
        if (this.targetElement.node && this.targetElement.node.dataset && this.targetElement.node.dataset.baseFontSize) {
            finalFontSize = parseFloat(this.targetElement.node.dataset.baseFontSize);
        }
        if (isNaN(finalFontSize) || finalFontSize <= 0) finalFontSize = parseFloat(this.targetElement.node.getAttribute('data-base-font-size'));
        if (isNaN(finalFontSize) || finalFontSize <= 0) {
            finalFontSize = parseFloat(this.targetElement.css('font-size') || this.targetElement.attr('font-size'));
        }
        if (isNaN(finalFontSize) || finalFontSize <= 0) {
        ...
        let lineHeight = finalFontSize * spacingVal;
        ...
        if (hasGroup && typeof window.SVGUtils !== 'undefined') {
            ...
            if (group.attr('data-annotation') === 'true' || (bgShape && bgShape.attr('data-annotation') === 'true')) {
                isAnnotation = true;
            }
        } else if (typeof window.SVGUtils !== 'undefined') {
            const assocId = this.targetElement.attr('data-associated-shape-id');
            if (assocId) {
                bgShape = window.currentEditingSVG ? window.currentEditingSVG.draw.findOne('#' + assocId) : SVG('#' + assocId);
                if (bgShape && bgShape.attr('data-annotation') === 'true') {
                    isAnnotation = true;
                }
            }
        }
        ...
        if (bgShape && typeof window.SVGUtils !== 'undefined') {
            ...
            // ▼追加: 吹き出しの場合は、尻尾を含まない正確な「矩形本体サイズ」を取得する
            if (bgShape.attr('data-tool-id') === 'bubble') {
                ...
                if (!isNaN(bw) && bw > 0) locW = bw;
                if (!isNaN(bh) && bh > 0) locH = bh;
            } else if (isNaN(locW) || locW <= 0) { // ▼ if を else if に変更
            ...
            if (locW > 0) {
                ...
                if (hasGroup) {
                    ...
                    sx = Math.sqrt(gMatrix.a * gMatrix.a + gMatrix.b * gMatrix.b);
                    sy = Math.sqrt(gMatrix.c * gMatrix.c + gMatrix.d * gMatrix.d);
                } else {
                    const sMatrix = bgShape.matrix();
                    sx = Math.sqrt(sMatrix.a * sMatrix.a + sMatrix.b * sMatrix.b);
                    sy = Math.sqrt(sMatrix.c * sMatrix.c + sMatrix.d * sMatrix.d);
                }
                
                let visualW = locW;
                let visualH = locH;
                
                // ※ locH が正確になったため、visualH = locH * 0.8 の強引な縮小処理は削除
                ...
                if (isAnnotation && typeof window.SVGUtils.wrapAndFitText === 'function') {
                    ...
                    fontOpts.fontSize = Math.max(finalFontSize, baseConfigFontSize);
                    
                    const fitResult = window.SVGUtils.wrapAndFitText(cleanText, visualW, visualH, fontOpts, spacingVal, 8);
                    lines = fitResult.lines;
                    finalFontSize = fitResult.fontSize;
                    // 対象テキスト要素のフォントサイズも更新する
                    this.targetElement.font({ size: finalFontSize });
                    
                    // フォントサイズが変わったので、lineHeight を更新
                    lineHeight = finalFontSize * spacingVal;
                    
                } else if (typeof window.SVGUtils.wrapAndTruncateText === 'function') {
        ...
        requestAnimationFrame(() => {
            ...
            if (window.SVGTextAlignmentToolbar && typeof window.SVGTextAlignmentToolbar.updateTextPosition === 'function') {
                ...
                const target = (parent && parent.attr && (parent.attr('data-tool-id') === 'shape-text-group' || parent.attr('data-tool-id') === 'bubble')) ? parent : targetEl;
