...
function startSVGEdit(container, svgIndex) {
    ...
    function isInternalElement(node) {
        ...
        const internalPatterns = [
            ...
            'svg-spacing-indicators', 'svg-control-marker', 'polyline-handle-group', 'svg-grad-skeleton-hitarea',
    ...
    try {
        ...
        if (wrapper) {
            ...
            buttons.forEach(b => {
                ...
                // 「編集」ボタンは隠さず「完了」ボタンに切り替え、その左に「拡大画面」ボタンを追加する
                if (b.classList.contains('code-edit-btn') &&
                    !b.classList.contains('btn-svg-external-edit') &&
                    !b.classList.contains('btn-paint-external-edit') &&
                    !b.classList.contains('btn-unwrap-svg') &&
                    !b.classList.contains('btn-save-svg') &&
                    !b.classList.contains('btn-svg-expand')) {
                    ...
                    b.classList.add('svg-done-mode');
                    b._originalOnclick = b.onclick;
                    b.onclick = (e) => {
                        e.stopPropagation();
                        stopSVGEdit();
                    };

                    // 「拡大画面」ボタンを動的に生成して左隣に挿入
                    if (!b.parentNode.querySelector('.btn-svg-expand')) {
                        const expandBtn = document.createElement('button');
                        expandBtn.className = 'code-edit-btn btn-svg-expand';
                        expandBtn.textContent = typeof I18n !== 'undefined' ? (I18n.translate('editor.expandView') || '拡大画面') : '拡大画面';
                        expandBtn.title = typeof I18n !== 'undefined' ? (I18n.translate('editor.expandView') || '拡大画面') : '拡大画面';
                        expandBtn.onclick = (e) => {
                            e.stopPropagation();
                            showSVGExpandedView();
                        };
                        b.parentNode.insertBefore(expandBtn, b);
                    }
                } else if (b.classList.contains('btn-svg-expand')) {
                    // 自信で追加した拡大画面ボタンは非表示にしない
                } else {
        ...
        // [FIX] data URI を含む <image> タグの xlink:href を退避
        // WebView2等や SVG.js (v3) の処理によってDOMのattributesから href が消失・空文字化する問題への対策
        // SVG.jsがadoptする前、いかなるDOM操作も行われる前の「一番最初の状態」で必ずバックアップを取る
        svgElement.querySelectorAll('image').forEach(elNode => {
            if (!elNode.hasAttribute('data-is-proxy')) {
                const href = elNode.getAttribute('href') || 
                             elNode.getAttribute('xlink:href') || 
                             elNode.getAttributeNS('http://www.w3.org/1999/xlink', 'href');
                if (href && href.startsWith('data:image/')) {
                    elNode.setAttribute('data-original-href-backup', href);
                }
            }
        });
        ...
        if (!svgElement.getAttribute('data-paper-width') && typeof normalizeSVGTagForPreviewWidth === 'function') {
            ...
            if (result.changed) {
                ...
                if (parsedSvg) {
                    ...
                    // Markdownソースにもサイレント同期（編集完了時に確定するため遅延実行、ただし履歴破壊を防ぐため履歴に追加）
                    setTimeout(() => {
        ...
        const calcW = vbBase.width ? Math.round(vbBase.width * initZoomFactor) : 820;
        const calcH = vbBase.height ? Math.round(vbBase.height * initZoomFactor) : 450;

        let domW = !isNaN(savedPaperW) ? savedPaperW : (parseFloat(svgElement.getAttribute('width')) || calcW);
        let domH = !isNaN(savedPaperH) ? savedPaperH : (parseFloat(svgElement.getAttribute('height')) || calcH);

        console.log(`[startSVGEdit DEBUG] viewBox.height=${vbBase.height}, initZoom=${initZoom}, calcH=${calcH}, savedPaperH=${savedPaperH}, domH=${domH}`);

        // [FIX] 前バージョンのバグによって data-paper-height が極端に小さな値 (29など) に書き換わってしまった場合の自己修復
        if (Math.abs(domH - calcH) > 50 && calcH >= 50 && domH < calcH * 0.5) {
            console.warn(`[startSVGEdit] Self-healing height: ${domH} is abnormally small compared to calculated ${calcH}. Restoring.`);
            domH = calcH;
            domW = calcW;
        }
        ...
        // [FIX] 前回のバグで保存された間違った viewBox や height 属性が `<svg>` タグに残ったままだと、
        // この後の canvasProxy 生成時や svg.resize プラグインの初期化時に、ブラウザのレイアウトエンジンが
        // 誤った viewBox に引っ張られて要素サイズを潰してしまう。
        // それを防ぐため、この時点で正しく計算された論理サイズ(baseW/baseH)から viewBox と height を強制的にリセットする。
        if (svgElement && current.baseWidth && current.baseHeight) {
            const configW = (typeof AppState !== 'undefined' && AppState.config && AppState.config.previewWidth) ? AppState.config.previewWidth : current.baseWidth;
            const displayH = Math.round(current.baseHeight * (configW / current.baseWidth));
            svgElement.setAttribute('width', configW);
            svgElement.setAttribute('height', displayH);
            
            // 初期のズーム値とオフセットがわかるまでは一旦基本の100%スケールで仮設定しておく
            svgElement.setAttribute('viewBox', `${current.baseX} ${current.baseY} ${current.baseWidth} ${current.baseHeight}`);
        }
        ...
        current.applyZoomPan = function () {
            ...
            this._zoomPanSyncTimer = setTimeout(() => {
                if (typeof syncChanges === 'function' && window.currentEditingSVG === this && !this._skipNextZoomSync) {
                    syncChanges(true, null, true);
                }
                this._skipNextZoomSync = false;
            }, 500);

            // [FIX] ズーム変更後に選択UI（8点ハンドル・回転ハンドル）の座標を再計算する
            // viewBox 変更後の getScreenCTM() が最新値を返すよう rAF で1フレーム遅延させる
            if (this.selectedElements && this.selectedElements.size > 0) {
                cancelAnimationFrame(this._zoomSelectionSyncRaf);
                this._zoomSelectionSyncRaf = requestAnimationFrame(() => {
                    if (!window.currentEditingSVG || window.currentEditingSVG !== this) return;
                    this.selectedElements.forEach(el => {
                        try {
                            ...
                            console.warn('[applyZoomPan] selection handle resync failed:', err);
                        }
                    });
                });
            }
         ...
         * @param {Array} spacings Array of { axis: 'x'|'y', x1, y1, x2, y2, gap }
         */
        current.showSpacingIndicators = function (spacings) {
            if (!this.spacingGroup || !this.spacingGroup.node.parentNode || this.spacingGroup.node.ownerSVGElement !== this.draw.node) {
                this.spacingGroup = this.draw.group().attr({
                    'class': 'svg-spacing-indicators',
                    'data-internal': 'true'
                }).front();
                this.spacingGroup.node.style.pointerEvents = 'none';
            }
            this.spacingGroup.clear();

            if (!spacings || spacings.length === 0) return;

            const color = '#ff4444';
            const width = 1 / (this.zoom / 100);
            const arrowSize = 4 / (this.zoom / 100);

            for (const sp of spacings) {
                // Draw main line
                this.spacingGroup.line(sp.x1, sp.y1, sp.x2, sp.y2)
                    .stroke({ color: color, width: width });

                // Draw arrows and '=' signs
                if (sp.axis === 'x') {
                    // Left arrow
                    this.spacingGroup.polyline([
                        [sp.x1 + arrowSize, sp.y1 - arrowSize],
                        [sp.x1, sp.y1],
                        [sp.x1 + arrowSize, sp.y1 + arrowSize]
                    ]).fill('none').stroke({ color: color, width: width });

                    // Right arrow
                    this.spacingGroup.polyline([
                        [sp.x2 - arrowSize, sp.y2 - arrowSize],
                        [sp.x2, sp.y2],
                        [sp.x2 - arrowSize, sp.y2 + arrowSize]
                    ]).fill('none').stroke({ color: color, width: width });

                    // '=' sign at the center
                    const cx = (sp.x1 + sp.x2) / 2;
                    const eqSize = 3 / (this.zoom / 100);
                    this.spacingGroup.line(cx - eqSize, sp.y1 - eqSize, cx + eqSize, sp.y1 - eqSize).stroke({ color: color, width: width });
                    this.spacingGroup.line(cx - eqSize, sp.y1 + eqSize, cx + eqSize, sp.y1 + eqSize).stroke({ color: color, width: width });
                } else {
                    // Top arrow
                    this.spacingGroup.polyline([
                    ...
                    // '=' sign at the center (rotate 90 deg for vertical)
                    const cy = (sp.y1 + sp.y2) / 2;
                    const eqSize = 3 / (this.zoom / 100);
                    this.spacingGroup.line(sp.x1 - eqSize, cy - eqSize, sp.x1 - eqSize, cy + eqSize).stroke({ color: color, width: width });
                    this.spacingGroup.line(sp.x1 + eqSize, cy - eqSize, sp.x1 + eqSize, cy + eqSize).stroke({ color: color, width: width });
                }
            }
            this.spacingGroup.front();
        };

        /**
         * Hide all equal spacing indicators
         */
        current.hideSpacingIndicators = function () {
            if (this.spacingGroup) {
                this.spacingGroup.clear();
            }
        };
        ...
        console.log(`[startSVGEdit DEBUG] canvasProxy created with width=${canvasProxy.attr('width')}, height=${canvasProxy.attr('height')}`);
        ...
        const getRightPos = (idx) => ({ top: (20 + idx * 30) + 'px', right: '-36px' });
        ...
        // [NEW] Animation Style Toolbar
        if (typeof createAnimationStyleToolbar !== 'undefined') {
            window.animationStyleToolbar = createAnimationStyleToolbar(container, {
        ...
        if (typeof createAnimationPathToolbar !== 'undefined') {
            window.animationPathToolbar = createAnimationPathToolbar(container, {
                position: getRightPos(3),
                borderColor: '#FF5722'
            });
        }

        // [NEW] Animation List Toolbar (連続再生リスト)
        if (typeof createAnimationListToolbar !== 'undefined') {
            window.animationListToolbar = createAnimationListToolbar(container, {
                position: getRightPos(4),
                borderColor: '#E65100'
            });
            // SVGに保存されたリストデータを復元
            window.animationListToolbar.loadFromSvg();
        }
            ...
            // [NEW] Arrow Keys: Nudge selected elements
                ...
                if (selectedSet && selectedSet.size > 0) {
                    ...
                    if (moved) {
                        ...
                        if (typeof window.updateSVGFloatingToolbar === 'function') window.updateSVGFloatingToolbar();
                    }
                }
            }
        };
        ...
        // [NEW] マーキー（範囲）選択の初期化：Shift+ドラッグ（選択ゼロ）で範囲選択を開始
        if (typeof initMarqueeSelect === 'function') {
            initMarqueeSelect(current, draw);
        }
                    ...
                    for (let i = 0; i < buffer.length; i++) {
                        ...
                        if (mutation.type === 'childList' || mutation.type === 'characterData') {
                            if (mutation.type === 'childList') {
                                const allNodesAreInternal = (nodeList) => {
                                    for (const n of nodeList) {
                                        if (n.nodeType !== 1) continue;
                                        if (n.getAttribute('data-internal') === 'true') continue;
                                        if (n.classList) {
                                            const cls = n.className.baseVal || n.className || '';
                                            if (typeof cls === 'string' && (
                                                cls.includes('svg-canvas-proxy') || cls.includes('svg-grid') ||
                                                cls.includes('svg_select') || cls.includes('svg-select') || cls.includes('svg-interaction') ||
                                                cls.includes('svg-snap-guides') || cls.includes('svg-ruler') ||
                                                cls.includes('svg-control-marker') || cls.includes('-handle-group') || cls.includes('svg-marquee')
                                            )) {
                                                continue;
                                            }
                                        }
                                ...
                                const addedInternal = allNodesAreInternal(mutation.addedNodes);
                                const removedInternal = allNodesAreInternal(mutation.removedNodes);
                                if (!addedInternal || !removedInternal) {
        ...
        current._skipNextZoomSync = true;
...
function stopSVGEdit(skipRender = false) {
    ...
    // [NEW] Floating Toolbar を非表示
    if (window.svgFloatingToolbar) {
        window.svgFloatingToolbar.hide();
    }
    ...
    // [NEW] Animation List Toolbar 破棄
    if (window.animationListToolbar) {
        window.animationListToolbar.destroy();
        window.animationListToolbar = null;
    }
    ...
    if (draw) {
        ...
        // Leave IDs intact to prevent breaking `<use>` and `clip-path` references.
        // The editorDiv will either be replaced by renderer.js or kept if no changes were made.
    }
    ...
    if (wrapper) {
        const buttons = wrapper.querySelectorAll('button, .language-label');
        buttons.forEach(b => {
            if (b.classList.contains('code-edit-btn') && b.classList.contains('svg-done-mode')) {
                const origText = b.getAttribute('data-original-text') || (typeof I18n !== 'undefined' ? I18n.translate('editor.edit') || '編集' : '編集');
                b.textContent = origText;
                b.removeAttribute('data-original-text');
                b.classList.remove('svg-done-mode');
                b.onclick = b._originalOnclick || null;
                delete b._originalOnclick;

                // 追加した拡大画面ボタンを削除
                const expandBtn = b.parentNode.querySelector('.btn-svg-expand');
                if (expandBtn) {
                    expandBtn.remove();
                }
            }
    ...
    // [NEW] マーキー選択用キャプチャリスナーのクリーンアップ
    if (window.currentEditingSVG._marqueeDownHandler && draw) {
        draw.node.removeEventListener('mousedown', window.currentEditingSVG._marqueeDownHandler, true);
    }
...
function importImageAsBase64(dataUrl, dropPoint = null) {
    ...
    try {
        ...
        img.on('load', function () {
            ...
            // [FIX] インポートした画像も属性消失対策の退避を行う
            const node = img.node;
            const href = node.getAttribute('href') || node.getAttribute('xlink:href');
            if (href && href.startsWith('data:image/')) {
                node.setAttribute('data-original-href-backup', href);
            }
...
window.getSVGContextMenuActions = function() {
    return {
        resetLayout: () => {
            ...
            if (window.currentEditingSVG && window.currentEditingSVG.gridToolbar && typeof window.currentEditingSVG.gridToolbar.resetPosition === 'function') {
                window.currentEditingSVG.gridToolbar.resetPosition();
            }
            if (window.currentEditingSVG && window.currentEditingSVG.transformToolbar && typeof window.currentEditingSVG.transformToolbar.resetPosition === 'function') {
                window.currentEditingSVG.transformToolbar.resetPosition();
            }
            if (window.currentEditingSVG && window.currentEditingSVG.alignToolbar && typeof window.currentEditingSVG.alignToolbar.resetPosition === 'function') {
            ...
            if (window.currentEditingSVG && window.currentEditingSVG.textAlignToolbar && typeof window.currentEditingSVG.textAlignToolbar.resetPosition === 'function') {
                window.currentEditingSVG.textAlignToolbar.resetPosition();
            }
            if (window.currentEditingSVG && window.currentEditingSVG.fontToolbar && typeof window.currentEditingSVG.fontToolbar.resetPosition === 'function') {
                window.currentEditingSVG.fontToolbar.resetPosition();
            }
            if (window.currentEditingSVG && window.currentEditingSVG.lineToolbar && typeof window.currentEditingSVG.lineToolbar.resetPosition === 'function') {
                window.currentEditingSVG.lineToolbar.resetPosition();
            }
            if (window.currentEditingSVG && window.currentEditingSVG.pathOpToolbar && typeof window.currentEditingSVG.pathOpToolbar.resetPosition === 'function') {
            ...
            if (window.animationListToolbar && typeof window.animationListToolbar.resetPosition === 'function') {
                window.animationListToolbar.resetPosition();
            }
            if (window.animationStyleToolbar && typeof window.animationStyleToolbar.resetPosition === 'function') {
                window.animationStyleToolbar.resetPosition();
            }
        },
        ...
        bringForward: typeof moveSelectedForward === 'function' ? moveSelectedForward : null,
        sendBackward: typeof moveSelectedBackward === 'function' ? moveSelectedBackward : null,
        ...
        addToAnimList: () => {
            if (!window.animationListToolbar) return;
            const sel = window.currentEditingSVG && window.currentEditingSVG.selectedElements;
            if (!sel || sel.size === 0) return;
            const el = Array.from(sel)[0];
            const domNode = el.node || el;
            window.animationListToolbar.addToSequence(domNode);
        },
...
function handleContextMenu(e, container, svgIndex) {
    ...
    const actions = window.getSVGContextMenuActions();
...
window.updateSVGFloatingToolbar = function(container, svgIndex) {
    if (!window.svgFloatingToolbar) return;

    if (!window.currentEditingSVG || 
        !window.currentEditingSVG.selectedElements || 
        window.currentEditingSVG.selectedElements.size === 0) {
        window.svgFloatingToolbar.hide();
        return;
    }
    ...
    // 現在のコンテナやインデックスがない場合は、SVGエディタ内を探すフォールバック
    const currentContainer = container || window.currentEditingSVG.container;
    const currentSvgIndex = svgIndex !== undefined ? svgIndex : 0; // 適当なデフォルト

    window.svgFloatingToolbar.show(
