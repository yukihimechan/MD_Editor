...
function updateRulers(draw) {
    ...
    const startX = Math.floor(x / minorInterval) * minorInterval;
    for (let curX = startX; curX <= x + w; curX += minorInterval) {
        if (curX < x) continue;
        let tickH = 4 * scale;
        let isMajor = false;
        let isMedium = false;

        if (Math.round(curX) % majorInterval === 0) {
            tickH = 12 * scale;
            isMajor = true;
        } else if (Math.round(curX) % mediumInterval === 0) {
            tickH = 8 * scale;
            isMedium = true;
        }
        ...
        if (isMajor) {
            rulerGroup.text(Math.round(curX).toString())
    ...
    const startY = Math.floor(y / minorInterval) * minorInterval;
    for (let curY = startY; curY <= y + h; curY += minorInterval) {
        if (curY < y) continue;
        let tickW = 4 * scale;
        let isMajor = false;
        let isMedium = false;

        if (Math.round(curY) % majorInterval === 0) {
            tickW = 12 * scale;
            isMajor = true;
        } else if (Math.round(curY) % mediumInterval === 0) {
            tickW = 8 * scale;
            isMedium = true;
        }
        ...
        if (isMajor) {
            rulerGroup.text(Math.round(curY).toString())
