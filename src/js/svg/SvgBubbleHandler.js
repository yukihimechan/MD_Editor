...
class SvgBubbleHandler {
    ...
    update(overlayGroup, node, bbox) {
        ...
        let bubblePath = null;
        if (node.getAttribute('data-tool-id') === 'bubble') {
            bubblePath = node;
        } else if (node.getAttribute('data-tool-id') === 'shape-text-group') {
            bubblePath = node.querySelector('[data-tool-id="bubble"]');
        }
