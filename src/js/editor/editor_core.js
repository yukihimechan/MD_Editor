...
function initEditor() {
    ...
    function getSliderDecorationsFromState(state) {
        ...
        // 項目4対応：SVGコードブロック内の行だけを処理対象にするため、
        // 先にSVGブロックの文字位置範囲を収集する
        const svgBlockRanges = [];
        const blockRegex = /```svg\s*\n[\s\S]*?\n```/g;
        let bm;
        while ((bm = blockRegex.exec(text)) !== null) {
            svgBlockRanges.push({ start: bm.index, end: bm.index + bm[0].length });
        }
        // SVGブロック内の位置か否かを判定するヘルパー
        const isInSvgBlock = svgBlockRanges.length > 0
            ? (pos) => svgBlockRanges.some(r => pos >= r.start && pos <= r.end)
            : () => true; // ブロックなしの場合は全体を対象（従来の振る舞い）
        ...
        while (m = attrRegex.exec(text)) {
            ...
            // SVGブロック内の属性のみスライダーを表示
            if (!isInSvgBlock(m.index)) continue;
        ...
        for (const line of lines) {
            ...
            // 項目4対応: SVGブロック外の行は数値列検出をスキップ（ドキュメント本文の日付等にスライダーが表示されるのを防ぐ）
            if (!isInSvgBlock(currentPos)) {
                currentPos += lineLength + 1;
                continue;
            }
            ...
            const numbers = []; // Initialize array for this line
            let match;
    ...
    const extensions = [
        ...
        EditorView.updateListener.of(update => {
            ...
            // [NEW] 共同編集中は Yjs が AppState.text の更新とプレビューレンダリングを管理するため、
            // ここでの直接更新をスキップする（collab_core.js の observeYText() に委ねる）
            // ただし「自分自身の入力」は Yjs → CM6 のバインディングで処理されるため問題ない
            if (typeof CollabManager !== 'undefined' && CollabManager.isActive()) {
                // AppState.text だけは即時更新しておく（他モジュールの参照用）
                AppState.text = update.state.doc.toString();
                AppState.isModified = true;
                // debounce timer による render() と Undo/Redo 更新はスキップ
                // （Yjs の observeYText から render が呼ばれる）
                return;
            }
            ...
            highlights.forEach(h => {
                ...
                while ((match = regex.exec(blockText)) !== null) {
                    if (currentOcc === occIndex) {
                        ...
                        if (!isSelfClosing) {
                            // 項目3対応: 解析前にコメント（<!-- -->）内の文字列をプレースホルダに置換して
                            // タグ探索への影響を除去する
                            const blockTextForSearch = blockText.replace(/<!--[\s\S]*?-->/g, (m) => ' '.repeat(m.length));

                            // 対応する閉じタグ（</tagName>）をネストを考慮して探す
                            ...
                            while (depth > 0) {
                                const nextOpenMatch = blockTextForSearch.substring(searchPos).match(openingTagPattern);
                                const nextClose = blockTextForSearch.indexOf(closingTag, searchPos);

                                if (nextClose === -1) break; // エラーまたは不完全なXML

                                if (nextOpenMatch && (searchPos + nextOpenMatch.index) < nextClose) {
                                    const openIndex = searchPos + nextOpenMatch.index;
                                    const openEnd = blockTextForSearch.indexOf('>', openIndex);

                                    if (openEnd !== -1 && blockTextForSearch.substring(openIndex, openEnd + 1).trim().endsWith('/>')) {
                                        // このネストした開きタグは自己閉じタグなのでdepthは増加しない
                                        searchPos = openEnd + 1;
                                    } else {
                                        depth++;
