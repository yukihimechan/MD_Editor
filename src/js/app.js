...
async function init() {
    ...
    if (typeof CollabUI !== 'undefined' && typeof CollabUI.bindEvents === 'function') {
        CollabUI.bindEvents();
    }
    ...
    // --- スタンドアロンエクスポート版の埋め込みMarkdown検出 ---
    // export_standalone.js によって </body> 前に埋め込まれたタグを優先的に処理する
    const embeddedMarkdownEl = document.getElementById('embedded-markdown');
    if (embeddedMarkdownEl && embeddedMarkdownEl.textContent.trim()) {
        try {
            // Base64デコード（encodeURIComponent でエンコードされているため、decodeURIComponent も必要）
            const encoded = embeddedMarkdownEl.textContent.trim();
            const mdText = decodeURIComponent(atob(encoded));

            if (typeof setEditorText === 'function') setEditorText(mdText);
            AppState.text = mdText;
            ...
            console.log('[Init] Loaded from embedded-markdown, launching in preview mode.');
            return; // 通常の起動処理をスキップ
        } catch (err) {
            console.error('[Init] Failed to load embedded-markdown:', err);
            // エラーが発生した場合は通常起動へフォールスルー
        }
    }
