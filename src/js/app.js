                        ...
                        try {
                            ...
                            if (isImage) {
                                if (window.currentEditingSVG && window.currentEditingSVG.draw) {
                                    ...
                                    if (typeof showToast === 'function') showToast("SVGエディタに配置しています...", "info");
                                    try {
                                        ...
                                        if (typeof showToast === 'function') showToast("インポートに失敗しました", "error");
                                    }
                                    continue; // SVGに配置したため、Markdownとしては処理せず次のファイルへ
                                } else {
                                    // Markdownエディタへの挿入
                                    if (typeof showToast === 'function') showToast("画像を挿入しています...", "info");
                            ...
                            if (typeof showToast === 'function') showToast("ファイルを読み込んでいます...", "info");
                            const text = await fs.readTextFile(filePath);
                            const fileName = await path.basename(filePath);
                            ...
                            if (typeof showToast === 'function') showToast(`ファイルの読み込みに失敗しました: ${err}`, "error");
                        }
                    }
                }
            };
            ...
            if (queryFilePath || urlParams.get('file') || urlParams.get('url')) {
                ...
                try {
                    ...
                    if (targetPath.startsWith('http://') || targetPath.startsWith('https://')) {
                        // Remote URL
                        if (typeof showToast === 'function') showToast("リモートファイルを読み込んでいます...", "info");
                    ...
                    if (typeof showToast === 'function') showToast(`ファイルの読み込みに失敗しました: ${err.message}`, "error");
                }
            } else {
