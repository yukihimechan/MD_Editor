...
class MermaidToolbar extends SVGToolbarBase {
    constructor(options = {}) {
        ...
        this._toolDefs = [
            ...
            },
            {
                id:    'doublecircle',
                label: 'ノードを追加（二重円）',
                icon:    `<circle cx="12" cy="12" r="8" stroke-width="2"/><circle cx="12" cy="12" r="5" stroke-width="2"/>`,
                viewBox: '0 0 24 24',
            },
            {
                id:    'circle',
                label: 'ノードを追加（円形）',
                icon:    `<circle cx="12" cy="12" r="8" stroke-width="2"/>`,
                viewBox: '0 0 24 24',
            },
            {
                id:    'cylinder',
                label: 'ノードを追加（円柱）',
                icon:    `<path d="M5 8 C5 5 19 5 19 8 L19 16 C19 19 5 19 5 16 Z" stroke-width="2"/><path d="M5 8 C5 11 19 11 19 8" stroke-width="2"/>`,
                viewBox: '0 0 24 24',
            },
            { isSeparator: true },
            {
            ...
            { isSeparator: true },
            {
                id:    'expand',
                label: '拡大表示（パン・ズーム対応）',
                icon:    `<path d="M15 3h6v6M9 21H3v-6M21 3l-7 7M3 21l7-7" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>`,
                viewBox: '0 0 24 24',
            }
        ];
    ...
    _addNode(diagramWrapper, toolId) {
        ...
        if (toolId === 'doublecircle') nodeLabel = '二重円';
        if (toolId === 'circle') nodeLabel = '円形';
        if (toolId === 'cylinder') nodeLabel = '円柱';
        ...
        if (toolId === 'subgraph') {
            ...
            if (toolId === 'diamond') {
                nodeText = `    ${nodeId}{${nodeLabel}}`;
            } else if (toolId === 'capsule') {
                nodeText = `    ${nodeId}(${nodeLabel})`;
            } else if (toolId === 'doublecircle') {
                nodeText = `    ${nodeId}(((${nodeLabel})))`;
