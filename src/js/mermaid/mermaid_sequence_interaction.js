...
window.MermaidSequenceInteraction = {
    ...
    _attachEvents(container, svg) {
        svg.addEventListener('dblclick', (e) => {
            ...
            if (!container.classList.contains('mermaid-sequence-edit-mode')) {
                ...
                if (classes.includes('actor') || classes.includes('participant') || parentClasses.includes('actor') || parentClasses.includes('participant')) {
        ...
        svg.addEventListener('contextmenu', (e) => {
            ...
            let isMsg = classes.includes('message') || parentClasses.includes('message');
            ...
            if (target.tagName.toLowerCase() === 'path' || target.tagName.toLowerCase() === 'line') {
                if (!classes.includes('actor-line')) isMsg = true;
            }
            if (target.tagName.toLowerCase() === 'text' || target.tagName.toLowerCase() === 'tspan') {
    ...
    _editLabelInline(container, svg, target, type) {
        ...
        if (targetNode.tagName.toLowerCase() === 'text') {
            ...
            const textNode = targetNode.tagName.toLowerCase() === 'g' ? targetNode.querySelector('text') : (targetNode.parentElement ? targetNode.parentElement.querySelector('text') : null);
            if (textNode) {
        ...
        if (type === 'message') {
            ...
            // テキストが空の場合（線をクリックされた場合）は最寄りのテキストを取得
            if (!currentText) {
                const textNodes = Array.from(svg.querySelectorAll('text'));
    ...
    _selectMessage(container, svg, target, clickedText, e) {
        ...
        if (window.activeMermaidSequenceToolbar) {
            window.activeMermaidSequenceToolbar.setSelectedMessage(textContentTrimmed, messageIndex);
        }
    },
