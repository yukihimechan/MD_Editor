...
class MermaidSequenceToolbar extends window.SVGToolbarBase {
    constructor() {
        ...
        this._toolDefs = [
            ...
            { isSeparator: true },
            {
                id:    'expand',
                label: '拡大表示（パン・ズーム対応）',
                icon:  `<path d="M15 3h6v6M9 21H3v-6M21 3l-7 7M3 21l7-7" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round"/>`,
                viewBox: '0 0 24 24',
            }
        ];
    ...
    _buildToolbar() {
        ...
        this._toolDefs.forEach(t => {
            if (t.isSeparator) {
                ...
                sep.style.cssText = 'width:1px; height:16px; background:rgba(0,0,0,0.1); margin:0 4px;';
    ...
    _restoreEditMode() {
        ...
        const diagramWrapper = document.querySelector(`.mermaid-diagram-wrapper[data-line="${this._lastEditedLine}"]`);
        if (diagramWrapper) {
