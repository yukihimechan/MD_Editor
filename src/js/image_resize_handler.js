...
const ImageResizeHandler = (() => {
    ...
    async function _updateImageInSource(newWidth, newMarginLeft) {
        ...
        // HTML 形式
        updated = updated.replace(htmlPattern, (match, src) => {
        ...
        if (!matched) {
            ...
            for (const target of targets) {
                let idx = updated.indexOf(target);
                if (idx !== -1) {
                    let startIdx = updated.lastIndexOf('![', idx);
                    if (startIdx !== -1) {
                        // 改行をまたがないことを確認（遠くの![を誤検知しないため）
                        let between = updated.substring(startIdx, idx);
                        if (!between.includes('\n')) {
                            let altText = updated.substring(startIdx + 2, idx);
                            _origAlt = altText || _origAlt;
                            const newImgTag = `<img src="${_origSrc}" width="${Math.round(newWidth)}"${styleAttr} alt="${_origAlt}">`;
                            updated = updated.substring(0, startIdx) + newImgTag + updated.substring(idx + target.length);
                            matched = true;
                            break;
                        }
                    }
