...
async function requestDirectoryAccess(targetFileName = null) {
    ...
    if (!window.showDirectoryPicker) {
        alert('お使いのブラウザはディレクトリアクセスAPIをサポートしていません。\n画像を表示するには、Chrome、Edge、またはOpera等をご使用ください。');
        return false;
    }
    ...
    try {
        ...
        if (savedDirHandle) {
            ...
            if (perm === 'prompt') {
                ...
                if (newPerm === 'granted') {
                    AppState.fileDirectory = savedDirHandle;
                    if (typeof showToast === 'function') showToast(`ディレクトリ "${AppState.fileDirectory.name}" へのアクセスが復元されました`);
                    return true;
                }
            ...
            try {
                ...
                if (typeof showToast === 'function') showToast(`ディレクトリ "${AppState.fileDirectory.name}" へのアクセスが許可されました`);
                return true;
            } catch (e) {
    ...
    try {
        ...
        const insertToApp = (insertText) => {
            if (clientX !== undefined && clientY !== undefined && target !== undefined &&
                ...
                if (typeof showToast === 'function') showToast('エディタの初期化に失敗しました', 'error');
            }
        };
            ...
            if (!isTauriEnv) {
                ...
                if (!window.showDirectoryPicker) {
                    if (typeof showToast === 'function')
                        showToast('お使いのブラウザはフォルダアクセスAPIに対応していません', 'error');
                ...
                if (saveLocation === 'local') {
                    ...
                    try {
                        ...
                        if (!mdIsSaved) {
                            ...
                            if (typeof showToast === 'function') {
                                showToast('MDファイルと画像ファイルを保存するフォルダを選択してください。', 'info');
                            }
                            ...
                            if (!dirHandle) {
                                if (typeof showToast === 'function') {
                                    showToast(`画像フォルダ操作のため、次に出るダイアログで現在のMDファイル「${AppState.fileHandle.name}」と同じフォルダを選択しアクセス権を付与してください。`, 'info');
                                }
                        ...
                        if (!mdIsSaved) {
                            ...
                            if (typeof showToast === 'function')
                                showToast(`「${defaultMdName}」で保存しました。`, 'success');
                        }
                        ...
                        if (e.name === 'AbortError') {
                            if (typeof showToast === 'function') showToast('フォルダの選択がキャンセルされました', 'warning');
                        } else {
                            console.error('Browser local save error:', e);
                            if (typeof showToast === 'function')
                                showToast('ローカルへの保存に失敗しました (文書内埋め込みに変更します)', 'warning');
                            saveLocation = 'document';
                        }
                ...
                if (!baseDir) {
                    ...
                    if (typeof showToast === 'function')
                        showToast('先にファイルを保存してください', 'warning');
        ...
        if (saveLocation === 'document') {
            if (typeof showToast === 'function') showToast(`${isSvg ? 'SVG' : '画像'}を読み込み中: ${file.name}...`, "info");
            const reader = new FileReader();
        ...
        if (typeof showToast === 'function') showToast('画像の読み込み・保存に失敗しました', 'error');
    }
}
...
async function openLinkedMarkdownFile(relativePath) {
    ...
    if (tauri) {
        ...
        if (tauri.webviewWindow && tauri.webviewWindow.WebviewWindow) {
            ...
            alert('新しいウィンドウを開く機能が見つかりません (WebviewWindow missing)');
            return;
        }
        ...
        try {
            ...
            if (!baseDir) {
                ...
                if (relativePath === './README.md' || relativePath === 'README.md') {
                    alert('サンプルページからのリンクは、実際にファイルを開いてからのみ機能します。\n先に「開く」ボタンからファイルを開くか、ファイルをドラッグ＆ドロップしてください。');
                } else {
                    alert('ベースディレクトリが不明なため、リンクを開けません。\nマークダウンファイルを保存するか、既存のファイルを開いてください。');
                }
                return;
            }
            ...
            alert('ファイルのオープンに失敗しました: ' + e.message);
        }
        return;
    }
    ...
    if (relativePath === './README.md' || relativePath === 'README.md') {
        ...
        try {
            ...
            if (!dirHandle) {
                ...
                if (!granted) {
                    if (typeof showToast === 'function') showToast("ディレクトリアクセスが拒否されました", "error");
                    return;
                }
            ...
            if (!fileHandle) {
                if (typeof showToast === 'function') showToast("選択したフォルダにREADME.mdが見つかりません", "error");
                return;
            }
            console.log('[Utils] Found fileHandle for README.md:', fileHandle.name);

            if (typeof showToast === 'function') showToast("README.mdを読み込んでいます...", "info");
            ...
            if (typeof showToast === 'function') showToast("「README.md」を新しいウィンドウで開きました");
            return;

        } catch (e) {
            console.error('Failed to load README.md:', e);
            if (typeof showToast === 'function') showToast("README.mdを読み込めませんでした", "error");
            return;
        }
    }

    if (!AppState.fileDirectory) {
        if (typeof showToast === 'function') showToast("リンクを開くには基準となるフォルダを選択してください", "info");

        const granted = await requestDirectoryAccess(fileName);
        if (!granted) {
            if (typeof showToast === 'function') showToast("フォルダが選択されなかったため、リンクを開けませんでした", "warning");
            return;
        }
    }

    try {
        if (typeof showToast === 'function') showToast("ファイルを読み込み中: " + relativePath, "info");

        const fileHandle = await resolveRelativePath(AppState.fileDirectory, relativePath);

        if (!fileHandle) {
            if (typeof showToast === 'function') showToast("ファイルが見つかりません: " + relativePath, "error");
            return;
        }
        ...
        if (typeof showToast === 'function') showToast(`「${file.name}」を新しいウィンドウで開きました`);

    } catch (e) {
        console.error('Failed to open linked file:', e);
        if (typeof showToast === 'function') showToast("ファイルを開けませんでした: " + e.message, "error");
    }
}
