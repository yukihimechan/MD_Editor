...
async function exportTextToFile() {
    ...
    if (!text) {
        if (typeof showToast === 'function') showToast("エクスポートする内容がありません", "error");
        return;
    }
    ...
    try {
        if (window.__TAURI__) {
            ...
            if (targetPath) {
                ...
                if (typeof showToast === 'function') showToast("テキストファイルを保存しました");
            }
            return;
        }
        ...
        if (window.showSaveFilePicker) {
            ...
            a.click();
            if (typeof showToast === 'function') showToast("テキストファイルをダウンロードしました");
        }
    } catch (e) {
        if (e.name !== 'AbortError') {
            console.error('Text export failed:', e);
            if (typeof showToast === 'function') showToast("エクスポートに失敗しました", "error");
        }
    }
}
...
async function exportTextToClipboard() {
    const text = getPreviewText();
    if (!text) {
        if (typeof showToast === 'function') showToast("コピーする内容がありません", "error");
        return;
    }
    ...
    try {
        ...
        try {
            ...
            if (typeof showToast === 'function') showToast("クリップボードにコピーしました (fallback)");
        } catch (err) {
            if (typeof showToast === 'function') showToast("コピーに失敗しました", "error");
        }
    }
}
